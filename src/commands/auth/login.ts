...
import { styled, UI, COLORS, APP_DESCRIPTION } from '../../config/constants';
...
export async function loginCommand(): Promise<void> {
  ...
  if (authService.isAuthenticated()) {
    ...
    if (current) {
      console.log(
        boxen(
          styled.success(`Already logged in as ${styled.accent(current.profile.username)}`),
          {
            padding: 1,
            margin: 1,
            borderStyle: 'round',
      ...
      const { action } = await inquirer.prompt([
        {
          ...
          choices: [
            { name: styled.secondary('Continue to Dashboard'), value: 'continue' },
  ...
  if (profiles.length === 0) {
    ...
    console.log(
      boxen(styled.info('No profiles found. Let\'s create your first profile!'), {
        padding: 1,
        margin: 1,
        borderStyle: 'round',
        borderColor: COLORS.BRIGHT_BLUE,
  ...
  const { selectedProfile } = await inquirer.prompt([
    {
      ...
      choices: [
        ...
        { name: styled.primary(`${UI.ICONS.ROCKET} Create New Profile`), value: '__new__' },
      ],
    },
  ]);
  ...
  if (result.success && result.profile) {
    console.log(
      boxen(
        styled.success(`${UI.ICONS.SUCCESS} Welcome back, ${styled.accent(result.profile.username)}!`),
        {
          padding: 1,
          margin: 1,
          borderStyle: 'round',
    ...
    console.log(
      boxen(styled.error(`${UI.ICONS.ERROR} ${result.error || 'Login failed'}`), {
        padding: 1,
        margin: 1,
        borderStyle: 'round',
...
async function createNewProfile(): Promise<void> {
  ...
  if (result.success && result.profile) {
    console.log(
      boxen(
        styled.success(
          `${UI.ICONS.SUCCESS} Profile created successfully!\nWelcome, ${styled.accent(result.profile.username)}!`
        ),
        {
          padding: 1,
          margin: 1,
    ...
    console.log(
      boxen(styled.error(`${UI.ICONS.ERROR} ${result.error || 'Failed to create profile'}`), {
        padding: 1,
        margin: 1,
        borderStyle: 'round',
        borderColor: COLORS.ERROR,
...
function displayBanner(): void {
  const logo = figlet.textSync('KARPI', {
    font: 'ANSI Shadow',
    horizontalLayout: 'default',
  });

  const gradientLogo = gradient([COLORS.BOTTLE_GREEN, COLORS.BRIGHT_BLUE])(logo);
